...
export default function AdminTestimonial() {
     ...
     function DeleteData(id) {
          if (window.confirm("Are you sure you want to delete details?")) {
               dispetch(deleteTestimonial({ id: id }))
               GetAPIData()
               ...
               <div className="container-fluid my-3">
                    <div className="row">
                         ...
                         <div className="col-xl-10 col-md-9">
                              <div className="row">
                                   <h5 className='bg-primary border-primary p-2 text-white text-center'>Testimonial<Link to='/admin/testimonial/create'><i className='fa fa-plus float-end text-white'></i></Link ></h5>
                                                       ...
                                                       data.map((item, index) => {
                                                            return <tr key={index}>
                                                                 ...
                                                                 <td><Link to={`${item.pic}`} target='_blank' rel='noreferrer'>
                                                                      <img src={`${item.pic}`} height={60} width={70} alt='Testimonial Logo' />
                                                                 </Link></td>
                                                                 <td >{`${item.caption?.slice(0,170) + "..."}`}</td>
